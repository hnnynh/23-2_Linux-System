...
{
	return ((struct task_struct *) elem)->cpu;
}

static void print_pxt4_file_write_iter_dm(unsigned long cpu_id,
		const char *name,
		unsigned long long count,
		int percentage)
{
	printk("cpu[%ld] called pxt_file_write_iter() %lld, time (%d%%)\n", cpu_id, count, percentage);
}

DEFINE_DS_MONITORING(pxt4_file_write_iter_dm, get_cpu_id, NULL, print_pxt4_file_write_iter_dm);
...
/*  original calclock
	...
	find_ds_monitoring(&pxt4_file_write_iter_dm, current);
